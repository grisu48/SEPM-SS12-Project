...
public class ISnippetImpl extends GWTSessionServlet implements ISnippet {
	...
	public List<XComment> getComments(long snippet, int start, int count) throws NotFoundException {
		...
		if (snip == null) throw new NotFoundException("Snippet with id " + snippet + " not found");
	...
	public void rateSnippet(long id, int rate) throws NoAccessException, NotFoundException {
		...
		Session session = getSession();
		if (!session.getPolicy().canRateSnippet(session, snippet)) throw new NoAccessException();
	...
	public void setDescription(long id, String desc) throws NoAccessException, NotFoundException {
		...
		Snippet snippet = findSnippetThrowsException(id);
		Session session = getSession();
		if (!session.getPolicy().canEditSnippet(session, snippet)) throw new NoAccessException();
	...
	public void setCode(long id, String code) throws NoAccessException, NotFoundException {
		...
		Code objCode = Code.createCode(code, snippet.getCode().getLanguage(), snippet);
	...
	public void addComment(long id, String comment) throws NoAccessException, NotFoundException {
		...
		if (snippet == null) throw new NotFoundException("Snippet with id " + id + " not found");
	...
	public void create(String name, String desc, String code, String language, String license, String category,
		...
		if (name == null || name.isEmpty()) throw new IllegalArgumentException("New snippet name cannot be empty");
		if (desc == null || desc.isEmpty()) throw new IllegalArgumentException("New snippet desc cannot be empty");
		if (code == null || code.isEmpty()) throw new IllegalArgumentException("New snippet code cannot be empty");
		...
		if (category != null && cat == null) throw new IllegalArgumentException("Category not found");

		Session session = getSession();
		if (!session.getPolicy().canCreateSnippet(session, cat)) throw new NoAccessException();

		User owner = session.getUser();
		if (owner == null) throw new NoAccessException();
		...
		try {
			Snippet result = Snippet.createSnippet(owner.getUsername(), name, desc, category, code, language, license,
					tagList);

			if (result == null) throw new RuntimeException("Unknown error");
		} catch (IOException e) {
	...
	private Snippet findSnippetThrowsException(long id) throws NotFoundException {
		Snippet snippet = Snippet.getSnippet(id);
		if (snippet == null) throw new NotFoundException("Snippet with id " + id + " could not be found");
		return snippet;
	}
	...
	public void edit(XSnippet snippet) throws NoAccessException, NotFoundException, IllegalArgumentException {
		...
		if (origin == null) throw new NotFoundException();
		if (!session.getPolicy().canEditSnippet(session, origin)) throw new NoAccessException();
		if (user == null) throw new NoAccessException();
	...
	public long getDownloadSourceTicket(long snippet_id) throws NotFoundException, NoAccessException {
		...
		Code code = snippet.getCode();
		if (code == null) throw new NotFoundException("Snippet does not contain any source codes");
	...
	public boolean canEdit(long snippet_id) throws NotFoundException {
		...
		if (!session.getPolicy().canEditSnippet(session, snippet)) return false;
		User user = session.getUser();
	...
	public XSnippet getSnippetOfDay() {
		...
		if (snippetofDay == null) return null;
